...
async def fetch_page_videos(provider: str, q: str, page: int, headers: dict) -> list:
    ...
    if provider == "pornhub":
        # 1. First attempt to scrape standard video container elements
        items = soup.select('li.videoblock, li.pcVideoListItem, div.videoBox, div.ph-thumb-container, li[data-video-vkey]')
        
        # 2. Fallback to extracting all viewkey anchor tags if structure changes dynamically
        if not items:
            items = soup.select('a[href*="viewkey="]')
        ...
        for item in items:
            ...
            if item.name == 'a':
                ...
                container = item.find_parent('li') or item.find_parent('div', class_=re.compile(r'video|wrap|block', re.I)) or item
                
                title_tag = container.select_one('.title a, a.title, .video-title, span.title, p.title a')
                if title_tag:
                ...
                if img_tag:
                    thumb = (img_tag.get('data-thumb_url') or 
                             img_tag.get('data-mediumthumb') or 
                             img_tag.get('data-image') or 
                             img_tag.get('data-src') or 
                             img_tag.get('src') or "")
            else:
                a_tag = item.select_one('a[href*="viewkey="]')
                if not a_tag:
                ...
                title_tag = item.select_one('span.title a, a.title, .title, a[title], img[alt]')
                if title_tag:
            ...
            if thumb and ('data:image' in thumb or '1x1' in thumb or 'tracking' in thumb or 'logo' in thumb):
                thumb = ""
...
async def search_provider_robust(provider: str, q: str, page: int):
    ...
    for v in raw_videos:
        ...
        if not vk and not u: continue
        if vk in seen_vkeys or u in seen_urls: continue
    ...
    if not unique_videos:
        try:
            search_url = ""
            if provider == "pornhub": search_url = f"https://www.pornhub.com/video/search?search={quote(q)}&page={page}"
            elif provider == "youporn": search_url = f"https://www.youporn.com/search/?query={quote(q)}&page={page}"
            elif provider == "xhamster": search_url = f"https://xhamster.com/search/{quote(q)}"
            elif provider == "redtube": search_url = f"https://www.redtube.com/?search={quote(q)}"
            elif provider == "xnxx": search_url = f"https://www.xnxx.com/search/{quote(q)}"
            elif provider == "xvideos": search_url = f"https://www.xvideos.com/?k={quote(q)}"
...
def parse_metadata_fallback(url: str, provider: str) -> dict:
    ...
    if "pornhub.com" in url: base_domain = "https://www.pornhub.com"
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if not scraped_title:
                    ...
                    if title_tag:
                        scraped_title = title_tag.get_text().replace('&amp;', '&').split('- RedTube')[0].split('- YouPorn')[0].strip()
                ...
                if view_match:
                    ...
                    if 'k' in view_match.group(0).lower(): view_count = int(float(raw_views.replace('k', '')) * 1000)
                    elif 'm' in view_match.group(0).lower(): view_count = int(float(raw_views.replace('m', '')) * 1000000)
                    else: view_count = int(raw_views) if raw_views.isdigit() else 0

                date_match = re.search(r'(\d{4}-\d{2}-\d{2})', resp.text)
                if date_match: upload_date = date_match.group(0)
...
def extract_with_ytdlp(url: str) -> dict:
    ...
    provider = "youporn"
    if "pornhub.com" in url: provider = "pornhub"
    elif "xhamster.com" in url: provider = "xhamster"
    ...
    for attempt in range(3):
        try:
            ...
            clean_thumbs = [t for t in all_thumbs if 'hash=' not in t and 'validto=' not in t and 'hdnea=' not in t and 'svg' not in t and 'logo' not in t]
