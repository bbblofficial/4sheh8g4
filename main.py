...
def search_pornhub_with_ytdlp(q: str, page: int) -> list:
    ...
    try:
            ...
            if info:
                for entry in info.get('entries', []):
                    ...
                    if not vkey and 'viewkey=' in url:
                        try: vkey = url.split("viewkey=")[1].split("&")[0]
                        except: pass
                    ...
                    title = html_parser.unescape(entry.get('title', f"PornHub Video {vkey}"))
        ...
        logger.error(f"yt-dlp pornhub fallback search error: {e}")
    return videos
...
def search_youporn_with_ytdlp(q: str, page: int) -> list:
    ...
    try:
            ...
            if info:
                for entry in info.get('entries', []):
                    ...
                    if not vkey and '/watch/' in url:
                        ...
                        if match: vkey = match.group(1)
                    if not vkey: continue

                    title = html_parser.unescape(entry.get('title', f"YouPorn Video {vkey}"))
                    thumb = entry.get('thumbnail', '')
                    full_url = url if url.startswith('http') else f"https://www.youporn.com{url}"
                    videos.append({
                        "vkey": vkey,
                        "title": title,
                        "thumbnail": thumb,
        ...
        logger.error(f"yt-dlp youporn fallback search error: {e}")
    return videos
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 200:
                ...
                if provider == "youporn":
                    items = soup.select('div.video-box, div.pb-card, div.list-item, li.video-tile, div.videoBox, div.videoListItem, div[class*="video"], a[href*="/watch/"]')
                    for item in items:
                        ...
                        if item.name == 'a':
                            ...
                            href = a_tag.get('href', '')
                            full_url = href if href.startswith('http') else f"https://www.youporn.com{href}"
                            title_tag = item.select_one('a[href*="/watch/"] [title], p.title, span.title, a, div.title')
                    ...
                    for item in items:
                        ...
                        img_tag = item.select_one('img')
                        thumb = img_tag.get('data-thumb_url') or img_tag.get('data-mediumthumb') or img_tag.get('data-src') or img_tag.get('src') or "" if img_tag else ""

                        videos.append({"vkey": vkey, "title": html_parser.unescape(title), "thumbnail": thumb, "url": full_url, "provider": "pornhub"})
                        if len(videos) >= 48: break

                elif provider == "xhamster":
                    items = soup.select('div.video-thumb, div.thumb-list__item, div.video-container, div.cell, article')
                    for item in items:
